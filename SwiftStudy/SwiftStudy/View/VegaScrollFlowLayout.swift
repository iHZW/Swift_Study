...
//        m41: 1, m42: 0, m43: 0, m44: 0)
    ...
    override open func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        dynamicItems?.forEach { item in
			...
			item.zIndex = index
            transformItemIfNeeded(y: convertedY, item: item, index:i)
            
        }
        return dynamicItems
    }
	
	private func transformItemIfNeeded(y: CGFloat, item: UICollectionViewLayoutAttributes, index:Int) {
...
//        print("index---\(index)")
        item.alpha = index != 1 ? 1 : alphaDistributor(x: y)
