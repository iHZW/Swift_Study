...
class SecondViewController: BaseViewController, InteractionViewControllerProtocol {
    func sendData(data: String) {
//        self.view.makeToast("-09-09-00--09-0")
        UIApplication.shared.keyWindow?.makeToast(data, position: .top)
...
//            self.navigationController?.pushViewController(ViewDetailPage(), animated: true)
            let vc = InteractionViewController(data: "第一个界面传值") {
                print("-=\($0)=-")
            }
            vc.delegate = self
//            vc.closure = {
//                print("closure = \($0)")
//            }
//            vc.closureBlock = { [unowned self](data: String) in
//                print("closureBlock = \(data), ttile =\(String(describing: self.title))")
//            }
